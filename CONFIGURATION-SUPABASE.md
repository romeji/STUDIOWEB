...
Le serveur envoie automatiquement : la confirmation questionnaire avec le délai annoncé de 48 h maximum, le lien privé dès la génération de l’aperçu, puis le récapitulatif de formule et du premier règlement après confirmation Stripe. Après le questionnaire, une fonction de fond appelle directement l’API OpenAI. Le HTML est enregistré dans Supabase, puis un lien de prévisualisation de 30 jours est créé et envoyé par e-mail. L’administration voit l’état, le modèle et l’usage de jetons, peut ouvrir l’aperçu dans le tableau de bord, et peut relancer une génération en échec. En cas d’indisponibilité du modèle, le client reçoit le délai de secours de 48 h et le brief reste consultable pour une reprise manuelle. Les photos jointes sont transmises au modèle avec les réponses; le nom, l’e-mail et le téléphone de suivi sont retirés du prompt. Vercel prolonge l’exécution de la fonction de fond via `waitUntil`; la durée de fonction est réglée à 60 secondes et l’appel du modèle s’arrête au bout de 40 secondes. Le redémarrage administrateur partage `/api/briefs` pour respecter la limite Hobby de 12 fonctions par déploiement.

Ajouter dans Vercel une clé API serveur `OPENAI_API_KEY` (ne jamais la placer dans le HTML ni dans le dépôt). Le modèle est configurable côté serveur par `AI_SITE_MODEL`, par défaut `gpt-6-luna`, appelé via l’API Responses d’OpenAI avec `store:false` et une sortie JSON structurée. La sortie est limitée à 12 000 jetons. L’appel n’est déclenché qu’après l’enregistrement d’un questionnaire valide. L’utilisation de l’API OpenAI est facturée à l’usage selon le compte API et séparément de tout abonnement ChatGPT; prévoir un budget ou des alertes dans la plateforme OpenAI. Au tarif standard publié actuellement, GPT-6 Luna coûte 0,10 $ par million de jetons d’entrée et 0,50 $ par million de jetons de sortie; le plafond de 12 000 jetons de sortie représente donc au plus environ 0,006 $ pour la sortie, auxquels s’ajoutent l’entrée et les images. Le compte API standard peut conserver les journaux de surveillance des abus jusqu’à 30 jours. Le traitement européen dédié nécessite l’éligibilité et l’activation des contrôles de données correspondants; il n’est pas présumé actif par le code.
