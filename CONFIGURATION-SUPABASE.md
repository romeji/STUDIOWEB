...
Ajouter dans Vercel une clé API serveur `OPENAI_API_KEY` (ne jamais la placer dans le HTML ni dans le dépôt). Le modèle est configurable côté serveur par `AI_SITE_MODEL`, par défaut `gpt-6-sol`, appelé via l’API Responses d’OpenAI avec `store:false` et une sortie JSON structurée. La sortie est limitée à 12 000 jetons. L’appel n’est déclenché qu’après l’enregistrement d’un questionnaire valide. L’utilisation de l’API OpenAI est facturée à l’usage selon le compte API et séparément de tout abonnement ChatGPT; prévoir un budget ou des alertes dans la plateforme OpenAI. Le compte API standard peut conserver les journaux de surveillance des abus jusqu’à 30 jours. Le traitement européen dédié nécessite l’éligibilité et l’activation des contrôles de données correspondants; il n’est pas présumé actif par le code.
